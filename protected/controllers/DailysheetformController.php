...
class DailysheetformController extends RController
{
    ...
    public function actionUpdate($id)
    {
        ...
        {
            $model->attributes=$_POST['Dailysheetform'];
            $model->total = $model->beginningcash + $model->beginningpaymaya + $model->beginning_gcash + $model->beginning_bpi + $model->beginning_ccg;
            if($model->save())
                $this->redirect(array('compute','id'=>$model->id));
        }
...
//        $print =str_replace("{paymayatotal}",$dailysheetform->beginningpaymaya,$print);
        ...
        $print =str_replace("{ccg_total}",number_format($dailysheetform->beginning_ccg, 2),$print);
        $valgrandtotal = $dailysheetform->beginningpaymaya + $dailysheetform->beginning_bpi + $dailysheetform->beginning_gcash + $dailysheetform->beginning_ccg + $dailysheetform->total;
        // var_dump($dailysheetform->beginning_ccg);
        ...
        $print =str_replace("{beginning_bpi}",$dailysheetform->beginning_bpi,$print);
        $print =str_replace("{beginning_ccg}",$dailysheetform->beginning_ccg,$print);
        //init
        $incsummary = array(
            'Laboratory' => 0.0,
            'Ancillary' => 0.0,
            ...
            if ($invoice != null)
            {
                ...
                {                   
                    ...
                    {   
                        ...
                        $sql = "SELECT * FROM  ref_productservice WHERE name = '$invoiceItem->description'";
                        ...
                        {  
                            ...
                            else if($productservice['provider']=="APE Bacoor LGU")
                                $incsummary['APE Bacoor LGU']=$incsummary['APE Bacoor LGU']+$invoiceItem->total;
        ...
        if($deposits!=null)
        {
            foreach ($deposits as $deposit)
            {
                ...
                if($deposit->category=="Credit Card / G-Cash")
                    $deposit_ccg = $deposit_ccg + $deposit->amount;
        ...
        $print = str_replace("{dccg_subtotal}",number_format($deposit_ccg,2),$print);
        $print = str_replace("{dotherstotal}",number_format($depositother,2),$print);
        $print = str_replace("{dtotal}",number_format($dailysheetform->cashdeposit+$depositmf+$depositjf+$deposithmo+$depositother+$depositpaymaya+$deposit_gcash+$deposit_bpi+$deposit_ccg,2),$print);
        //census
